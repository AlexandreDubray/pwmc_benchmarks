...
def pcnf_encoding(dataset):
    ...
    with open(os.path.join(script_dir, 'uai', dataset + '.uai')) as f:
        ...
        # First, we count the number of probabilistic variables needed
        variable_index = 1
        for cpt_idx in range(number_network_vars):
            variable_index += len([float(x) for x in lines[start_cpt + 2*cpt_idx + 1].rstrip().split(' ')])
        ...
        for cpt_idx in range(number_network_vars):
            ...
            for d in range(number_distribution):
                ...
                distributions.append(f'c p distribution {" ".join([str(x) for x in distribution_proba])}')
